---
title: "Exercicio: A1"
author: "Jairo Nicolau"
date: "9/20/2020"
output: html_document
---

Entrega no dia 27 de setembro às 20h; Subir no e-class.

### Carregar os pacotes

```{r}
library(tidyverse)
library(mosaicData)
```

### Carregar o banco de dados

```{r}
data("CPS85")
View(CPS85)
```


### 1) Casos e variáveis

Quantas variáveis e casos tem no banco? Quantas são variáveis quantitativas, quantas são variávies categóricas


```{r}

```

- resposta: 


### 2) Escolha uma variável categórica e filtre segundo um critério de sua escolha:

```{r}

```

- Quantos casos permaneceram?


### 3) Escolha uma variável quantitativa e filtre segundo um critério de sua escolha:

```{r}

```


### 4) Renomeie duas variáveis

```{r}

```

### 5) Crie uma nova variável a partir de duas outras quantitativas e selecione apenas essa variável

```{r}

```

### 6) Selecione uma variável quantitativa e calcule a média e mediana, segundo uma outra variável categórica

```{r}

```


### 7) Faça um gráfico de barra com uma variável categórica (se preferir, ordene as barras)

```{r}

```

### 8) Utlize o mesmo gráfico acima, agora renomeando as categorias (labels) para o português:

```{r}

```

### 9) Escolha uma variável quantitativa e faça um histograma:

```{r}

```

### 10) Refaça o gráfico acima, mudando o número de "bins" do histograma.

```{r}

```

- Existe alguma diferença relevante entre as figuras 9 e a 10? Se sim, qual?


